...
from data.miniwebvision import Webvision
from common.utils import init_seeds
import numpy as np
# import cv2
...
def build_webfg_dataset(root, train_transform, test_transform, number= None):
    train_data = IndexedImageFolder(os.path.join(root, 'train'), transform=train_transform, number=number)
    test_data = IndexedImageFolder(os.path.join(root, 'val'), transform=test_transform, number=number)
    return {'train': train_data, 'test': test_data, 'n_train_samples': len(train_data.samples), 'n_test_samples': len(test_data.samples)}


def build_miniwebvision_dataset(root, train_transform, test_transform):
    train_data = Webvision(root, transform=train_transform, mode='all', num_class=50 )
    test_data = Webvision(root, transform=test_transform, mode='test', num_class=50 )
    return {'train': train_data, 'test': test_data, 'n_train_samples': len(train_data.train_imgs), 'n_test_samples': len(test_data.val_imgs)}
